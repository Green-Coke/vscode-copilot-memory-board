...
| `debugWebviews: false` | 关闭 webview 浏览器调试 | ⚠️ js-debug 的运行时默认值 `extensionHostConfigDefaults.debugWebviews` 其实是 `false`,`OPTIONS.md` 中也声明为 `false`;但 VS Code 设置面板的 JSON schema(`generate-contributions.ts`)中声明了 `default: true`,会在 UI 展示上误导。若用户从设置面板改回 `true`,js-debug 会 attach Chrome/Edge 调 webview,被 Windows 弹窗策略拦截,弹"浏览器阻止了新窗口"对话框,故在此显式锁定为 `false` |
| `debugWebWorkerHost: false` | 同上,关闭 web worker 扩展宿主调试 | 同上,运行时默认是 `false`,但 schema 声明为 `true`,显式锁 `false`。**特别说明**:若扩展 `package.json` 声明 `extensionKind: ["web"]`,js-debug 的 resolver 会**动态**把 `debugWebWorkerHost` 改为 `true`(见 `extensionHostConfigurationResolver.ts#L46-51`)。本项目 `extensionKind` 非 web-only,不受影响 |
...
**根因**(经 `microsoft/vscode-js-debug` 源码三层验证):
- `OPTIONS.md` 与运行时 `extensionHostConfigDefaults` 默认值**都为 `false`**;
- 但 JSON schema(`generate-contributions.ts`)中声明 `debugWebviews: { default: true }` / `debugWebWorkerHost: { default: true }`,该 schema 用于 VS Code 设置面板 UI 展示;
- 一旦用户在设置面板被 schema 误导改成 `true`,F5 时 js-debug 便会启动 Chrome/Edge attach 到 webview,被 Windows 弹窗策略拦截。
- 额外:扩展若 `extensionKind: ['web']`,resolver 会把 `debugWebWorkerHost` 动态改为 `true`(本项目不受影响)。

**修复**:`launch.json` 显式设 `debugWebviews: false` 和 `debugWebWorkerHost: false`,把行为从"受 schema 影响"转为"代码锁死",避免环境漂移。
